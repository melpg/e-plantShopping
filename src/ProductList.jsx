...
function ProductList() {
    ...
    // Get total quantity from the Redux cart state
  const cartItems = useSelector((state) => state.cart.items);
  const totalQuantity = cartItems.reduce((total, item) => total + item.quantity, 0);
  useEffect(() => {
    // Reset the addedToCart state when cartItems change
...
const handlePlantsClick = (e) => {
  ...
  const handleAddToCart = (product) => {
    const sanitizedProduct = {
        ...product,
        cost: product.cost.replace('Rs', ''), // Remove the dollar sign
      };
      dispatch(addItem(sanitizedProduct));
  ...
  const handleRemoveFromCart = (product) => {
    dispatch(removeItem(product)); // Remove item from Redux store
  };
    return (
        <div>
